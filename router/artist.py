...
async def get_artist_releases(id: int, session: AsyncSession = Depends(get_db_session_async)) -> Optional[list[ArtistReleaseSchema]]:
    ...
    query = select(
                ...
                func.to_char((func.coalesce(func.sum(Track.duration).over(partition_by=Release.id), 0)*text("interval '1 sec'")), 'hh24:mi:ss').label('duration'),
                func.round((func.coalesce(func.sum(Track.file_size).over(partition_by=Release.id)/1024/1024, 0)), 2).label('size')).\
            distinct().\
